...
Em \citeonline{chen2021}, motivações sobre o uso da aplicação de GANs na área da saúde são apresentadas, assim como métricas de avaliação dos modelos e os desafios de adoção.

Em \citeonline{xu2019}, vemos uma implementação de GAN que alcança privacidade diferencial adicionando cuidadosamente ruído aos gradientes durante a fase de aprendizagem.

Em \citeonline{yoon2020}, uma nova abordagem é explorada para gerar dados sintéticos que respeitam as correlações existentes nos dados originais, além de propor uma equação para quantificar o quão identificável são os dados gerados.

Em \citeonline{wang2021}, são discutidos os requisitos para utilização de dados sintéticos para múltiplos propósitos na área de saude, além de sugerir uma abordagem para gerar e avaliar dados sintéticos.

Em \citeonline{heyburn2018}, é feita uma comparação entre diferentes métricas de avaliação produzidas por modelos treinados em dados sintéticos e modelos treinados em dados reais.

Em \citeonline{khaled2020}, sete abordagens para avaliação da utilidade estatísticas de conjuntos de dados sintéticos são discutidas.



Este \textit{template} apresenta regras básicas para a elaboração do trabalho segundo as normas ABNT. Estas regras devem ser seguidas rigorosamente a fim de que o mesmo possa receber sua ficha catalográfica e ser posteriormente aprovado para publicação na Biblioteca Digital de Teses e Dissertações (BDTD) da USP. Qualquer desvio realizado nas configurações e recomendações deste \textit{template} poderá causar atrasos nesses processos, uma vez que o texto precisará ser corrigido antes. 
